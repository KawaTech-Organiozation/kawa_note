...
- **AGENT_EXECUTOR:** Execução de planos aprovados no escopo `apps/api/`.

## Escopo Permitido
- ✅ Modificar código em `apps/api/src/`
- ✅ Modificar configurações em `apps/api/src/config/`
- ✅ Modificar schemas Prisma em `apps/api/prisma/`
- ✅ Modificar migrations em `apps/api/prisma/migrations/`
- ✅ Modificar `apps/api/package.json` (dependências)
- ❌ Modificar código em `apps/web/src/` (frontend)
...
- Não modificar `apps/api/.env` (variáveis sensíveis).
- Não modificar `apps/api/Dockerfile` sem aprovação explícita.
- Não modificar `docker-compose.local.yml` sem aprovação explícita.
- Migrations devem ser criadas via Prisma CLI, nunca manualmente.

## Sincronização de Documentação
A cada execução, verificar:
- `apps/api/package.json` vs `/.Agent/backend/.Stack.md`
- `apps/api/src/` vs `/.Agent/backend/.Architecture.md`
